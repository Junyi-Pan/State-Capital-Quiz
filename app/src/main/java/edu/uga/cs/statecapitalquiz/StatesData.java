...
public class StatesData {
    ...
    private static StatesData instance;
    Cursor c = null;
    ...
    public static StatesData getInstance(Context context) {
        if (instance == null) {
            instance = new StatesData(context);
        }
        return instance;
    }
    ...
    // Open the database
    public void open() {
        this.db = statesDbHelper.getWritableDatabase();
    ...
    public String getStateName(int id) {
        c=db.rawQuery("SELECT stateName FROM states WHERE id = '"+id+"'", new String[]{} );
        StringBuffer buffer = new StringBuffer();
        while (c.moveToNext()) {
            String stateName = c.getString(0);
            buffer.append(""+stateName);
        }
        return buffer.toString();
    }

    public String getCaptial(int id) {
        c=db.rawQuery("SELECT capital FROM states WHERE id = '"+id+"'", new String[]{} );
        StringBuffer buffer = new StringBuffer();
        while (c.moveToNext()) {
            String stateName = c.getString(0);
            buffer.append(""+stateName);
    ...
    public String getCity1(int id) {
        c=db.rawQuery("SELECT city1 FROM states WHERE id = '"+id+"'", new String[]{} );
        StringBuffer buffer = new StringBuffer();
        while (c.moveToNext()) {
            String stateName = c.getString(0);
            buffer.append(""+stateName);
    ...
    public String getCity2(int id) {
        c=db.rawQuery("SELECT city2 FROM states WHERE id = '"+id+"'", new String[]{} );
        StringBuffer buffer = new StringBuffer();
        while (c.moveToNext()) {
            String stateName = c.getString(0);
            buffer.append(""+stateName);
    ...
    //    Log.d(DEBUG_TAG, "before store state");
    //    long id = db.insert( StatesDBHelper.TABLE_STATES, null, values );
    //    Log.d(DEBUG_TAG, "after store state");
